...
# reduce size
# change types
#print(dist.info(memory_usage='deep'))
for column in dist:
    if dist[column].dtype == 'float64':
        dist[column]=pd.to_numeric(dist[column], downcast='float')
    if dist[column].dtype == 'int64':
        dist[column]=pd.to_numeric(dist[column], downcast='integer')
#print(dist.info(memory_usage='deep'))
dist['dest_type'] = dist['dest_type'].astype('category')
#print(dist.info(memory_usage='deep'))
dist['geoid'] = dist['geoid'].astype('int32')
#print(dist.info(memory_usage='deep'))


dist.to_csv('./data/results/parcel_distances.csv.zip', compression="zip")
