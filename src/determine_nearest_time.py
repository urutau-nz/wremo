...
def main(db):
    ...
    queries_1 = ['DROP TABLE IF EXISTS nearest_block;',
        ...
        # """,
        'CREATE TABLE IF NOT EXISTS nearest_block(geoid TEXT, dest_type TEXT, distance INT, duration INT, population INT, time INT, geometry geometry)'
    ]
    queries_2 = [''' INSERT INTO nearest_block (geoid, dest_type, distance, duration, population, time, geometry)
            SELECT dist.id_orig as geoid, destinations.dest_type, MIN(dist.distance) as distance, MIN(dist.duration) as duration, blocks."C18_CURPop" as population, dist.time as time, blocks.geometry
            ...
            GROUP BY dist.id_orig, destinations.dest_type, blocks.geometry, blocks."C18_CURPop", dist.time;
